...
Characters that can't be used in a folder name are replaced with `_`: path separators (`/`, `\`), control characters, and `:` (which would otherwise split the bind mount specification). A title such as `Wolfenstein: The New Order` therefore stores its data under `Wolfenstein_ The New Order`.
